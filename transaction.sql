...
--        RAISE INFO 'v_lower %,v_upper %, %',v_lower,v_upper,current_date;
  --      PERFORM  pg_sleep(10);
   -- exit;
WITH deleted AS (
DELETE FROM sch_cs_ff_storeoperations_inventory_service.transaction s
WHERE ctid IN (
    SELECT ctid
    FROM sch_cs_ff_storeoperations_inventory_service.transaction
    WHERE created_at < v_date
      AND  id % 10 BETWEEN v_lower AND v_upper
    LIMIT v_commit_size
    )
    RETURNING  id,created_at,created_by,is_deleted,modified_at,modified_by,"version",current_free_quantity,current_quantity,inventory_id,offer_id,owner_id,product_id,quantity,reference_id,related_transaction_id,seller_source,shelf_quantity,storage_location_id,store_id,transaction_quantity,transaction_source,"type"
    )
INSERT INTO archive.transaction (id,created_at,created_by,is_deleted,modified_at,modified_by,"version",current_free_quantity,current_quantity,inventory_id,offer_id,owner_id,product_id,quantity,reference_id,related_transaction_id,seller_source,shelf_quantity,storage_location_id,store_id,transaction_quantity,transaction_source,"type")
SELECT id,created_at,created_by,is_deleted,modified_at,modified_by,"version",current_free_quantity,current_quantity,inventory_id,offer_id,owner_id,product_id,quantity,reference_id,related_transaction_id,seller_source,shelf_quantity,storage_location_id,store_id,transaction_quantity,transaction_source,"type" FROM deleted;
