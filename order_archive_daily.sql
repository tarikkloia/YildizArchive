--        set time zone 'Europe/Istanbul';
        ...
        DECLARE
                v_month           INT2 := 13; --13
                v_commit_size     INT4 := 2;
                v_max_loop        INT2 := 5;
                v_limit_sec       INT2 := 10800;
                v_sleep_ms        INT2 := 0000;
        ...
        --PERFORM  pg_sleep(3);
--raise info 'bitti';
--PERFORM pg_sleep(1 + floor(random() * 5));
--if v_lower in (2,6) then
--raise info 'err';
--RAISE EXCEPTION 'Bu bir özel hatadır!';
--end if;
--v_total_count:=(v_lower+1)*(v_lower+1);
--v_duration:= make_interval(mins := floor(random() * 120 + 1)::int);
        --exit;
                        RAISE INFO '';
				...
				if v_lower in (3) and v_loop >2 then
				--raise info 'err';
				RAISE EXCEPTION 'Bu bir özel hatadır!';
end if;
