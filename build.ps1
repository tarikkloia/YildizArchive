...
$outputcreate = awslocal lambda create-function `
  ...
  --region us-east-1;Start-Sleep -Seconds 8 ;$output = awslocal lambda invoke --function-name test-go-lambda --cli-binary-format raw-in-base64-out --payload file://payload.json output.txt; Get-Content output.txt
